...
複数の模擬をまたいだ**個人点**は `/sum` でまとめる。
各模擬の結果を**右クリック → アプリ →「集計に追加」**していき（最大 5 模擬）、
最後に `/sum` を実行する。コピペは不要。合計は gb2 が計算する。

誤読を直したい場合は、直した表を自分でチャットに投稿し、そちらを「集計に追加」する。
Discord の外（テキストエディタなど）にテキストがある場合は、何も追加していない状態で
`/sum` を実行するとモーダルが開くのでそこに貼る。
