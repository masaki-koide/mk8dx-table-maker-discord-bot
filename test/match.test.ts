...
test("同じ長さで競合したら前方一致が勝つ", () => {
  expect(matchTag("AほげB", ["A", "B"])).toBe("A");
  // 末尾にたまたま `a` を含む名前。後方優先だと `A` チームに吸われていた
  expect(matchTag("cocoa", ["A", "B", "C", "D"])).toBe("C");
});

test("前方一致が無ければ後方一致を使う", () => {
  expect(matchTag("cocoa", ["A", "B", "D"])).toBe("A");
});
